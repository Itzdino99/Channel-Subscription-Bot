...
def final_requested_admin_router(message):
    ...
    if t == FINAL_ADMIN_USERS:
        return bot.send_message(ADMIN_ID, "👥 *Users*\n\nUse the existing user management tools below.", parse_mode="Markdown")
    if t == FINAL_ADMIN_PREMIUM:
        return bot.send_message(ADMIN_ID, "📺 *Premium & Channels*\n\nManage premium channels and subscriptions using the existing Premium/Channel tools.", parse_mode="Markdown")
    if t == FINAL_ADMIN_COINS:
        return bot.send_message(ADMIN_ID, "🪙 *Coins & Referrals*\n\nExisting coin, referral, milestone and leaderboard management remains available.", parse_mode="Markdown")
    if t == FINAL_ADMIN_REWARDS:
        return bot.send_message(ADMIN_ID, "🎁 *Rewards*\n\nDaily rewards and reward configuration remain available through the existing reward tools.", parse_mode="Markdown")
    if t == FINAL_ADMIN_TICKETS:
        rows = list(support_tickets_col.find({"status":"open"}).sort("created_at", DESCENDING).limit(20))
        text = "🎟️ *Support Tickets*\n\n" + ("No open tickets." if not rows else "\n".join(f"• #{r.get('ticket_id', r.get('_id',''))} — User `{r.get('user_id')}`" for r in rows))
        return bot.send_message(ADMIN_ID, text, parse_mode="Markdown")
    if t == FINAL_ADMIN_BROADCAST:
        return bot.send_message(ADMIN_ID, "📢 *Broadcast & Announcements*\n\nUse the existing broadcast and announcement tools.", parse_mode="Markdown")
    if t == FINAL_ADMIN_COUPONS:
        return bot.send_message(ADMIN_ID, "🎟️ *Coupons*\n\nUse the existing coupon management tools.", parse_mode="Markdown")
    if t == FINAL_ADMIN_OFFERS:
        return bot.send_message(ADMIN_ID, "🏷️ *Offers & Discounts*\n\nExisting discount and bulk-discount tools remain available.", parse_mode="Markdown")
    if t == FINAL_ADMIN_SETTINGS:
        return bot.send_message(ADMIN_ID, "⚙️ *Bot Settings*\n\nFeature controls, timezone and other existing settings remain available.", parse_mode="Markdown")
    if t == FINAL_ADMIN_STATS:
        return bot.send_message(ADMIN_ID, admin_dashboard_text(), parse_mode="Markdown")
    if t == FINAL_ADMIN_LOGS:
        return bot.send_message(ADMIN_ID, "📜 *Activity Logs*\n\nUse the existing audit/activity log tools.", parse_mode="Markdown")
