...
def ultimate_user_menu_markup(user_id=None):
    ...
    if adv_flag("daily_checkin"):
        markup.row(KeyboardButton(USER_DAILY))
    if adv_flag("lucky_spin"):
        markup.row(KeyboardButton(USER_SPIN), KeyboardButton(USER_DEALS))
    if adv_flag("vip_levels"):
        markup.row(KeyboardButton(USER_VIP), KeyboardButton(USER_DASHBOARD))
    if adv_flag("renewal"):
        markup.row(KeyboardButton(USER_RENEW), KeyboardButton(USER_GIFT))
...
def ultimate_admin_menu_markup():
    """Compact admin home screen. Detailed controls open inside submenus."""
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)

    # Main categories — kept compact like the user panel.
    markup.row(
        KeyboardButton(ADMIN_CHANNELS),
        KeyboardButton(ADMIN_PREMIUM),
    )
    markup.row(
        KeyboardButton(ADMIN_MILESTONES),
        KeyboardButton(ADMIN_VERIFICATION),
    )
    markup.row(
    ...
    markup.row(
        KeyboardButton(ADMIN_SETTINGS),
        KeyboardButton(ADMIN_STATISTICS),
    )
    markup.row(
        KeyboardButton(ADMIN_ADVANCED),
        KeyboardButton(ADMIN_MODE),
    )
    return markup
