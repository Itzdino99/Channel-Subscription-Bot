...
DEFAULT_SETTINGS = {
    ...
    # Bulk discount tiers. Each tier applies when the selected channel count is reached.
    # Example: [{"min_channels": 2, "type": "percent", "value": 10}]
    "bulk_discounts": [],
...
def redeem_channel_menu(user_id, chat_id):
    ...
    # Bulk redemption uses the normal single-channel plan price multiplied by
    # selected channels, with any matching admin discount tier applied.
    if len(channels) >= 2:
        markup.add(
            InlineKeyboardButton(
                "📦 Bulk Buy Premium",
...
def calculate_bulk_discount(channel_count, plan_cost):
    """Calculate the bulk total from normal plan price and the best matching admin discount tier."""
    subtotal = int(plan_cost) * int(channel_count)
    tiers = get_settings().get("bulk_discounts", []) or []
    applicable = []
    for tier in tiers:
        try:
            if int(channel_count) >= int(tier.get("min_channels", 0)):
    ...
    tier = sorted(applicable, key=lambda x: int(x.get("min_channels", 0)), reverse=True)[0]
    value = max(0, float(tier.get("value", 0)))
    if tier.get("type") == "percent":
        discount = int(round(subtotal * value / 100))
    else:
        discount = int(round(value))
    discount = min(discount, subtotal)
    return subtotal - discount, discount, tier

def bulk_price_summary(channel_count, plan_cost):
    subtotal = int(plan_cost) * int(channel_count)
    final, discount, tier = calculate_bulk_discount(channel_count, plan_cost)
    return subtotal, discount, final, tier
...
def bulk_redeem_start(call):
    ...
    for plan in plans:
        markup.add(
            InlineKeyboardButton(
                f"🎁 {format_duration(plan['amount'], plan['unit'])} — {plan['cost']} {settings['coin_name']} / channel",
...
📢 Select multiple Premium channels and redeem them together.
💡 The normal price is multiplied by the number of channels. Any discount tier set by the admin is automatically applied.

First, choose the Premium duration:""",
        reply_markup=markup,
        parse_mode="Markdown"
    )
...
def show_bulk_channel_selector(chat_id, user_id, plan_id):
    ...
    count = len(selected)
    subtotal, discount, final_price, tier = bulk_price_summary(count, plan["cost"]) if count else (0, 0, 0, None)
    markup.add(
        InlineKeyboardButton(
            f"🎉 Confirm Bulk Redeem ({final_price} {settings['coin_name']})",
            callback_data=f"bulkconfirm:{plan_id}"
        )
    )

    discount_text = f"{discount} {settings['coin_name']}" if discount else "No discount"
    bot.send_message(
        chat_id,
        f"""📦 *Select Premium Channels*

🎁 *Duration:* {format_duration(plan['amount'], plan['unit'])}
💰 *Price per Channel:* {plan['cost']} {settings['coin_name']}
📌 *Selected:* {count} channel(s)
💵 *Before Discount:* {subtotal} {settings['coin_name']}
🎁 *Discount:* {discount_text}
✅ *Final Price:* {final_price} {settings['coin_name']}
...
def confirm_bulk_redeem(call):
    ...
    try:
        ...
        bulk_price, discount_amount, discount_tier = calculate_bulk_discount(len(selected_ids), plan["cost"])
        subtotal_price = int(plan["cost"]) * len(selected_ids)
        ...
        # Deduct the calculated total after the matching bulk discount.
        result = bot_users_col.update_one(
            {
                "user_id": user_id,
        ...
        try:
            ...
            # Refund the calculated bulk price if any channel cannot be completed.
        ...
        lines = [
            ...
            f"💵 *Before Discount:* {subtotal_price} {settings['coin_name']}",
            f"🎁 *Discount:* {discount_amount} {settings['coin_name']}",
...
def admin_premium_menu(message):
    ...
    markup.add(
        InlineKeyboardButton(
            ...
            callback_data="premium:bulk_discounts"
        )
    )
...
def admin_users_menu(message):
    ...
    markup.add(
        InlineKeyboardButton("➕ Add Coins to User", callback_data="users:addcoins"),
        InlineKeyboardButton("👤 Single User Broadcast", callback_data="users:singlebroadcast")
    )

    markup.add(
        InlineKeyboardButton("🎁 Premium Buyers List", callback_data="users:premiumbuyers")
    )
...
@bot.callback_query_handler(func=lambda c: c.data == "premium:bulk_discounts")
def bulk_discounts_menu(call):
    if not admin_only(call): return
    tiers = get_settings().get("bulk_discounts", []) or []
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("➕ Add Discount Tier", callback_data="bulkdiscount:add"))
    for index, tier in enumerate(tiers):
        label = f"{tier.get('min_channels', 0)}+ channels — {tier.get('value', 0)}{'%' if tier.get('type') == 'percent' else ' coins'}"
        markup.add(InlineKeyboardButton(f"🗑️ {label}", callback_data=f"bulkdiscount:delete:{index}"))
    bot.send_message(ADMIN_ID, "🎁 *Bulk Discount System*\n\nSet discounts based on how many Premium channels a user selects. The highest matching channel tier is applied automatically.\n\nExample: 2+ = 10%, 4+ = 100 coins.", reply_markup=markup, parse_mode="Markdown")
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "bulkdiscount:add")
def bulk_discount_add(call):
    if not admin_only(call): return
    msg=bot.send_message(ADMIN_ID, "Send the minimum number of channels for this discount tier.\nExample: `2`", parse_mode="Markdown")
    bot.register_next_step_handler(msg, bulk_discount_get_count)
    bot.answer_callback_query(call.id)

def bulk_discount_get_count(message):
    try:
        count=int(message.text.strip())
        if count < 2: raise ValueError
    except Exception:
        bot.send_message(ADMIN_ID, "❌ Send a valid number (2 or more).")
        return
    markup=InlineKeyboardMarkup(row_width=2)
    markup.add(InlineKeyboardButton("📊 Percentage (%)", callback_data=f"bulkdiscount:type:{count}:percent"), InlineKeyboardButton("🪙 Fixed Coins", callback_data=f"bulkdiscount:type:{count}:fixed"))
    bot.send_message(ADMIN_ID, "Choose the discount type:", reply_markup=markup)

@bot.callback_query_handler(func=lambda c: c.data.startswith("bulkdiscount:type:"))
def bulk_discount_type(call):
    if not admin_only(call): return
    _,_,count,dtype=call.data.split(":")
    msg=bot.send_message(ADMIN_ID, f"Send the {'percentage' if dtype == 'percent' else 'coin amount'} discount for {count}+ channels.\nExample: `10`", parse_mode="Markdown")
    bot.register_next_step_handler(msg, bulk_discount_save, int(count), dtype)
    bot.answer_callback_query(call.id)

def bulk_discount_save(message, count, dtype):
    try:
        value=float(message.text.strip())
        if value < 0 or (dtype == 'percent' and value > 100): raise ValueError
    except Exception:
        bot.send_message(ADMIN_ID, "❌ Please send a valid discount value.")
        return
    tiers=get_settings().get("bulk_discounts", []) or []
    tiers=[t for t in tiers if int(t.get("min_channels", 0)) != count]
    tiers.append({"min_channels": count, "type": dtype, "value": value})
    tiers.sort(key=lambda x:int(x.get("min_channels",0)))
    update_setting("bulk_discounts", tiers)
    unit="%" if dtype == "percent" else f" {get_settings()['coin_name']}"
    bot.send_message(ADMIN_ID, f"✅ Bulk discount saved: *{count}+ channels → {value:g}{unit} discount*", parse_mode="Markdown")

@bot.callback_query_handler(func=lambda c: c.data.startswith("bulkdiscount:delete:"))
def bulk_discount_delete(call):
    if not admin_only(call): return
    try:
        index=int(call.data.rsplit(":",1)[1]); tiers=get_settings().get("bulk_discounts", []) or []
        if index < 0 or index >= len(tiers): raise ValueError
        tiers.pop(index); update_setting("bulk_discounts", tiers)
        bot.answer_callback_query(call.id, "Discount tier deleted.")
        bulk_discounts_menu(call)
    except Exception:
        bot.answer_callback_query(call.id, "Unable to delete this tier.", show_alert=True)
...
# =========================================================
# EXTRA ADMIN USER TOOLS
# =========================================================

@bot.callback_query_handler(func=lambda c: c.data == "users:addcoins")
def admin_add_coins_menu(call):
    if not admin_only(call): return
    msg=bot.send_message(ADMIN_ID, "Send: `USER_ID COIN_AMOUNT`\nExample: `123456789 100`", parse_mode="Markdown")
    bot.register_next_step_handler(msg, admin_add_coins_process)
    bot.answer_callback_query(call.id)

def admin_add_coins_process(message):
    try:
        user_id, amount=map(int, message.text.split())
        if amount <= 0: raise ValueError
        result=bot_users_col.update_one({"user_id":user_id}, {"$inc":{"coins":amount}})
        if result.matched_count == 0:
            bot.send_message(ADMIN_ID, "❌ User not found."); return
        bot.send_message(ADMIN_ID, f"✅ *{amount} {get_settings()['coin_name']}* added to user `{user_id}`.", parse_mode="Markdown")
        try: bot.send_message(user_id, f"🎉 Admin added *{amount} {get_settings()['coin_name']}* to your balance!", parse_mode="Markdown")
        except Exception: pass
    except Exception:
        bot.send_message(ADMIN_ID, "❌ Invalid format. Use: USER_ID COIN_AMOUNT")

@bot.callback_query_handler(func=lambda c: c.data == "users:singlebroadcast")
def single_broadcast_menu(call):
    if not admin_only(call): return
    msg=bot.send_message(ADMIN_ID, "Send the User ID you want to message.")
    bot.register_next_step_handler(msg, single_broadcast_get_user)
    bot.answer_callback_query(call.id)

def single_broadcast_get_user(message):
    try:
        user_id=int(message.text.strip())
        if not bot_users_col.find_one({"user_id":user_id}): raise ValueError
    except Exception:
        bot.send_message(ADMIN_ID, "❌ User not found or invalid User ID."); return
    msg=bot.send_message(ADMIN_ID, f"📩 Now send the message you want to send to `{user_id}`. Any message type can be copied.", parse_mode="Markdown")
    bot.register_next_step_handler(msg, single_broadcast_send, user_id)

def single_broadcast_send(message, user_id):
    try:
        bot.copy_message(user_id, message.chat.id, message.message_id)
        bot.send_message(ADMIN_ID, f"✅ Message sent to `{user_id}`.", parse_mode="Markdown")
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ Could not send the message to `{user_id}`.", parse_mode="Markdown")

@bot.callback_query_handler(func=lambda c: c.data == "users:premiumbuyers")
def premium_buyers_list(call):
    if not admin_only(call): return
    now_ts=datetime.now().timestamp()
    buyers=list(users_col.find({"expiry":{"$gt":now_ts}}).sort("expiry", DESCENDING).limit(100))
    if not buyers:
        bot.send_message(ADMIN_ID, "🎁 No active Premium buyers found."); bot.answer_callback_query(call.id); return
    lines=["🎁 Active Premium Buyers", ""]
    for i,item in enumerate(buyers,1):
        uid=item.get("user_id"); user=bot_users_col.find_one({"user_id":uid}) or {}
        name=user.get("first_name") or "User"; channel=premium_channels_col.find_one({"channel_id":item.get("channel_id")}) or {}
        lines += [f"{i}. {name}", f"🆔 {uid}", f"📢 {channel.get('name','Premium Channel')}", f"⏰ Expires: {format_bot_time(datetime.fromtimestamp(item['expiry']))}", ""]
    # Telegram messages have a length limit. Split safely.
    text="\n".join(lines)
    for start in range(0,len(text),3500): bot.send_message(ADMIN_ID,text[start:start+3500])
    bot.answer_callback_query(call.id)
