...
def advanced_renew_menu(message):
    """Show actual renewable Premium subscriptions and let the user extend them."""
    if not adv_flag("renewal"):
        bot.send_message(message.chat.id, "🚫 Renewal is disabled by Admin.")
        return
    uid = message.from_user.id
    now = bot_time_now()
    rows = list(users_col.find({"user_id": uid, "expiry": {"$exists": True}}).sort("expiry", DESCENDING).limit(50))
    if not rows:
        bot.send_message(message.chat.id, "🔄 Renew Premium\n\nNo Premium subscription was found. Open 🎁 Premium → 🎁 Redeem Premium to purchase one first.")
        return

    # One row per channel, using the latest expiry.
    latest = {}
    for row in rows:
        cid = row.get("channel_id")
        if cid is None:
            continue
        if cid not in latest or float(row.get("expiry", 0) or 0) > float(latest[cid].get("expiry", 0) or 0):
            latest[cid] = row
    markup = InlineKeyboardMarkup(row_width=1)
    for cid, row in latest.items():
        ...
        state = "ACTIVE" if exp > now.timestamp() else "EXPIRED"
        markup.add(InlineKeyboardButton(
...
def renew_premium_plan(call):
    ...
    try:
        ...
        if not plan or not channel:
            ...
            return
        cost = int(plan["cost"])
        result = bot_users_col.update_one({"user_id": uid, "coins": {"$gte": cost}, "banned": {"$ne": True}}, {"$inc": {"coins": -cost}})
        if result.modified_count != 1:
        ...
        try:
            ...
            users_col.update_one({"user_id": uid, "channel_id": cid}, {"$set": {"expiry": expiry.timestamp(), "source": "premium_renewal", "plan_id": plan_id, "duration": format_duration(plan["amount"], plan["unit"]) }}, upsert=True)
            log_purchase(uid, "premium_renewal", cost, get_settings()["coin_name"], {"channel_id": cid, "plan_id": plan_id})
            markup = InlineKeyboardMarkup()
            ...
            delivery_message = bot.send_message(uid, f"🔄 Premium Renewed Successfully!\n\n📢 Channel: {channel.get('name', 'Premium Channel')}\n🎁 Added: {format_duration(plan['amount'], plan['unit'])}\n⏰ New Expiry: {format_bot_time(expiry)}\n\n🔗 New join link:\n{link.invite_link}", reply_markup=markup, disable_web_page_preview=True)
            track_premium_delivery_message(uid, delivery_message.message_id, [cid])
        except Exception as e:
            add_coins(uid, cost)
            print(f"Renew premium error: {e}")
            bot.answer_callback_query(call.id, "❌ Renewal failed. Coins refunded.", show_alert=True)
    except Exception as e:
        print(f"Renew plan callback error: {e}")
        bot.answer_callback_query(call.id, "❌ Unable to renew Premium.", show_alert=True)
...
def final_ui_router(message):
    ...
    if uid == ADMIN_ID:
        ...
        if not is_admin_mode(uid) and text in {
            ...
            USER_FEEDBACK, USER_REDEEM, USER_GIFT, USER_RENEW, USER_BACK
        }:
            pass
...
def final_requested_admin_router(message):
    ...
    if t == FINAL_ADMIN_PREMIUM:
        ...
        return bot.send_message(ADMIN_ID, "📺 *Premium & Channels*\n\nManage both the original paid-channel system and the referral Premium system.", reply_markup=markup, parse_mode="Markdown")
    if t == FINAL_ADMIN_COINS:
        ...
        markup.row(InlineKeyboardButton("📊 Referral Stats", callback_data="users:stats"), InlineKeyboardButton("🔙 Back", callback_data="admin:back"))
    ...
    if t == FINAL_ADMIN_SETTINGS:
        markup = InlineKeyboardMarkup(row_width=2)
        markup.row(InlineKeyboardButton("🎛️ Feature Controls", callback_data="final:features"), InlineKeyboardButton("🕒 Timezone", callback_data="settings:timezone"))
        markup.row(InlineKeyboardButton("🪙 Coin & Referral", callback_data="settings:coins"), InlineKeyboardButton("✏️ Bot Texts", callback_data="settings:texts"))
