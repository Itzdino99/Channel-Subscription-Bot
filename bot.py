...
def get_settings():
    ...
    # Add newly introduced default settings automatically
    update = {}
...
def show_force_join(chat_id):
    ...
    if not channels:
        bot.send_message(
            chat_id,
            "⚠️ Verification channels have not been configured yet.\n"
    ...
    bot.send_message(
        chat_id,
        "🎉 *Welcome!*\n\n"
        "To complete your registration through the referral link, "
        "please join all the channels/groups below.\n\n"
        "After joining them, click *✅ Verify & Continue*.",
...
def is_user_in_channel(channel_id, user_id):
    try:
        ...
        # Telegram statuses that mean the user is still a member
...
def check_all_force_channels(user_id):
    ...
    # No channels means there is nothing to verify
    if not channels:
        return False
...
def start_handler(message):
    ...
    start_argument = text[1].strip() if len(text) > 1 else None

    # -----------------------------------------------------
    # 1. OLD PAID CHANNEL DEEP LINK
    # Channel IDs are normally negative (-100...)
    # -----------------------------------------------------
...
• Read all instructions before making a payment.""",
    ...
    # If a user already has a pending referral, NEVER bypass it
    if user_data and user_data.get("pending_referrer") is not None \
    ...
    if start_argument:
        try:
            ...
            if (
                ...
                # IMPORTANT:
                # We only SAVE the referral here.
                # NO coins and NO referral count yet.
                bot_users_col.update_one(
                    {"user_id": user_id},
                    {
                        "$set": {
...
def verify_referral(call):
    ...
    # Only users with a pending referral should verify
    ...
    # -----------------------------------------------------
    # ATOMIC-LIKE ONE-TIME VERIFICATION CHECK
    # -----------------------------------------------------
...
Thank you for inviting a new user!""",
        parse_mode="Markdown"
    )
...
def my_balance(message):
    ...
    bot.send_message(
        message.chat.id,
        f"""{settings['coin_emoji']} *My Balance*
...
📌 Your friend must join the required channels and successfully verify before you receive the reward.""",
        parse_mode="Markdown"
    )
...
Only users who completed verification are counted.""",
        parse_mode="Markdown"
    )
...
{get_settings()['coin_emoji']} Your balance: *{get_coin_balance(message.from_user.id)} {settings['coin_name']}*
...
def redeem_premium(call):
    ...
    # Atomically deduct only if enough coins
    result = bot_users_col.update_one(
        {
            "user_id": user_id,
            "coins": {"$gte": cost}
    ...
    try:
        ...
        # SAME COLLECTION AS OLD SUBSCRIPTION SYSTEM
        # kick_expired_users() will automatically remove them
        ...
        bot.answer_callback_query(call.id, "Premium redeemed!")
...
After your Premium time ends, you will automatically be removed from the Premium channel.""",
        ...
        # Refund if invite link creation fails
...
# =========================================================
# OLD USER PAYMENT FLOW - UNCHANGED
# =========================================================
...
Once you upload it, it will be forwarded to the admin.
...
🗑 `/forcelist` — View/remove verification channels

🎁 `/setpremium` — Set referral reward Premium channel
...
def save_force_channel(message):
    ...
    if chat.username:
        ...
        # For private groups/channels bot must be admin
        try:
...
⏰ Expires in: {hours} hours""",
        ...
        bot.reply_to(
            message,
            "❌ Coins, maximum users and hours must be numbers."
        )
...
def list_coupons(message):
    ...
    coupons = list(coupons_col.find().sort(
        "created_at",
        DESCENDING
    ).limit(20))
...
def kick_expired_users():
    ...
    for user in expired_users:
        ...
        try:
            ...
            # Different message for reward memberships
...
🪙 You can earn more coins through referrals and redeem Premium again!""",
                    parse_mode="Markdown"
                )
            ...
            # Don't delete the database record if Telegram failed.
            # It will try again on the next scheduler run.
