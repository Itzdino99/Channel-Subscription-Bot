...
# =========================================================
# SIMPLIFIED ADMIN PANEL BUTTONS
# =========================================================
ADMIN_USERS = "👥 Users"
...
def admin_menu_markup():
    """Main admin keyboard. Detailed controls are opened from these sections."""
    markup = ReplyKeyboardMarkup(
        resize_keyboard=True,
        row_width=2
    )
    ...
    markup.row(
        ...
        KeyboardButton(ADMIN_PREMIUM_CHANNELS)
    )
    markup.row(
        KeyboardButton(ADMIN_COINS_REFERRALS),
        KeyboardButton(ADMIN_REWARDS)
    )
    markup.row(
        KeyboardButton(ADMIN_SUPPORT_TICKETS),
        KeyboardButton(ADMIN_BROADCAST_ANNOUNCEMENTS)
    )
    markup.row(
        KeyboardButton(ADMIN_COUPONS),
        KeyboardButton(ADMIN_OFFERS)
    )
    markup.row(
        KeyboardButton(ADMIN_BOT_SETTINGS),
        KeyboardButton(ADMIN_STATISTICS)
    )
...
@bot.message_handler(
    func=lambda m: (
        ...
        and m.text not in {
            ...
            ADMIN_CAMPAIGNS, ADMIN_BROADCAST_TARGET, ADMIN_GIFTS, ADMIN_USERS, ADMIN_PREMIUM_CHANNELS, ADMIN_COINS_REFERRALS, ADMIN_SUPPORT_TICKETS, ADMIN_BROADCAST_ANNOUNCEMENTS, ADMIN_COUPONS, ADMIN_OFFERS, ADMIN_BOT_SETTINGS, ADMIN_STATISTICS, ADMIN_ACTIVITY_LOGS, ADMIN_STATISTICS,
...
def _admin_menu_buttons():
    return [
        ...
        ADMIN_SECURITY, ADMIN_NOTIFY, ADMIN_RECOVERY, ADMIN_CAMPAIGNS, ADMIN_BROADCAST_TARGET, ADMIN_GIFTS, ADMIN_USERS, ADMIN_PREMIUM_CHANNELS, ADMIN_COINS_REFERRALS, ADMIN_SUPPORT_TICKETS, ADMIN_BROADCAST_ANNOUNCEMENTS, ADMIN_COUPONS, ADMIN_OFFERS, ADMIN_BOT_SETTINGS, ADMIN_STATISTICS, ADMIN_ACTIVITY_LOGS,
...
def ultimate_admin_menu_markup(admin_id=None, page=None):
    """Authoritative compact Admin Panel keyboard."""
    buttons = [
        ADMIN_USERS, ADMIN_PREMIUM_CHANNELS,
        ADMIN_COINS_REFERRALS, ADMIN_REWARDS,
        ADMIN_SUPPORT_TICKETS, ADMIN_BROADCAST_ANNOUNCEMENTS,
        ADMIN_COUPONS, ADMIN_OFFERS,
        ADMIN_BOT_SETTINGS, ADMIN_STATISTICS,
        ADMIN_ACTIVITY_LOGS,
    ]
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    for i in range(0, len(buttons), 2): markup.row(*[KeyboardButton(x) for x in buttons[i:i+2]])
...
def final_user_pages(user_id):
    """Authoritative main user keyboard. Submenus are opened separately."""
    ensure_final_feature_flags()
    page = []
    if final_flag("profile"): page.append(USER_PROFILE)
    if final_flag("referrals"): page.append(USER_REFER)
    if final_flag("extra_features", True): page.append(USER_EXTRA)
    if final_flag("premium_menu", True): page.append(USER_PREMIUM)
    if final_flag("my_referrals"): page.append(USER_REFERRALS)
    if final_flag("leaderboard"): page.append(USER_LEADERBOARD)
    if final_flag("contact_admin"): page.append(USER_CONTACT)
    return [page] if page else [[USER_PROFILE]]
...
def extra_features_keyboard(user_id):
    ...
    if final_flag("announcements"): items.append(USER_ANNOUNCEMENTS)
    if final_flag("feedback"): items.append(USER_FEEDBACK)
    for i in range(0, len(items), 2): markup.row(*[KeyboardButton(x) for x in items[i:i+2]])
    markup.row(KeyboardButton(USER_BACK))
    return markup
...
def premium_keyboard(user_id):
    ...
    items = []
    if final_flag("premium_redeem") and (final_flag("single_redeem") or final_flag("bulk_redeem")): items.append(USER_REDEEM)
    if final_flag("gift_premium"): items.append(USER_GIFT)
    if final_flag("renewal"): items.append(USER_RENEW)
    for x in items: markup.row(KeyboardButton(x))
    markup.row(KeyboardButton(USER_BACK))
    return markup
...
def final_profile_text(message):
    ...
    s = get_settings()
    referrer_text = "None"
    rid = user.get("referrer_id")
    if rid:
        r = get_user(rid)
        referrer_text = ("@" + r.get("username")) if r and r.get("username") else (r.get("first_name", "User") if r else str(rid))
    lines = [
        "👤 *MY PROFILE*", "",
        f"👤 Name: *{message.from_user.first_name or 'User'}*",
        f"🆔 Telegram ID: `{uid}`",
        f"📅 Joined: *{user.get('joined_at') or user.get('created_at') or 'Unknown'}*", "",
    ...
    active = [r for r in rows if r.get("expiry") and float(r.get("expiry",0)) > datetime.now().timestamp()]
    lines.append("💎 *PREMIUM STATUS*")
    lines.append("✅ Active" if active else "❌ No Active Premium")
    if active:
        lines.append("")
        lines.append("📺 *Purchased Channels:*")
        for r in active:
            exp = r.get("expiry")
            try: exp_text = format_bot_time(datetime.fromtimestamp(float(exp), tz=bot_time_now().tzinfo))
            except Exception: exp_text = "Unknown"
            ch = premium_channels_col.find_one({"channel_id": r.get("channel_id")}) or {}
            lines.append(f"• {ch.get('name', r.get('channel_name','Premium Channel'))} — ⏰ {exp_text}")
    else:
        lines.append("")
        lines.append("📺 Purchased Channels: *None*")
    return "\n".join(lines)
...
def _final_ui_router_match(message):
    ...
    labels = {
        ...
        USER_MY_REWARDS, USER_FEEDBACK, USER_CONTACT, USER_SPIN, USER_NEXT, USER_PREVIOUS, USER_BACK, USER_EXTRA, USER_PREMIUM,
        ...
        ADMIN_CAMPAIGNS, ADMIN_BROADCAST_TARGET, ADMIN_GIFTS, ADMIN_USERS, ADMIN_PREMIUM_CHANNELS, ADMIN_COINS_REFERRALS, ADMIN_SUPPORT_TICKETS, ADMIN_BROADCAST_ANNOUNCEMENTS, ADMIN_COUPONS, ADMIN_OFFERS, ADMIN_BOT_SETTINGS, ADMIN_STATISTICS, ADMIN_ACTIVITY_LOGS,
    }
...
def final_ui_router(message):
    ...
    if uid == ADMIN_ID:
        ...
        admin_routes = {
            ADMIN_USERS: globals().get("admin_users_menu"),
            ADMIN_PREMIUM_CHANNELS: globals().get("admin_channels_menu"),
            ADMIN_COINS_REFERRALS: globals().get("admin_coins_referrals_menu"),
            ADMIN_REWARDS: globals().get("advanced_rewards_menu"),
            ADMIN_SUPPORT_TICKETS: globals().get("admin_support_tickets_menu"),
            ADMIN_BROADCAST_ANNOUNCEMENTS: globals().get("admin_broadcast_menu"),
            ADMIN_COUPONS: globals().get("admin_coupons_menu"),
            ADMIN_OFFERS: globals().get("admin_offers_menu"),
            ADMIN_BOT_SETTINGS: globals().get("advanced_admin_hub"),
            ADMIN_STATISTICS: globals().get("admin_statistics_menu"),
            ADMIN_ACTIVITY_LOGS: globals().get("admin_activity_logs_menu"),
        }
    ...
    if text == USER_NEXT:
        return final_show_user_menu(uid)
    if text in (USER_PREVIOUS, USER_BACK):
        return final_show_user_menu(uid)
    routes = {
        USER_EXTRA: lambda m: bot.send_message(m.chat.id, "🪄 *Extra Features*", reply_markup=extra_features_keyboard(uid), parse_mode="Markdown"),
        USER_PREMIUM: lambda m: bot.send_message(m.chat.id, "🎁 *Premium*", reply_markup=premium_keyboard(uid), parse_mode="Markdown"),
...
# =========================================================
# FINAL ROBUST FEATURE HANDLERS
# =========================================================
# These wrappers intentionally sit at the end of the source so the final UI
# router resolves these names at runtime. They keep the existing database and
# feature logic, but prevent malformed/legacy data from turning a button click
# into the generic "feature encountered an error" response.
...
def final_profile_text(message):
    ...
    try:
        ...
        settings = {}
    try:
        level, spend = get_vip_level(uid)
        if not isinstance(level, dict):
            level = {"name": "Member", "discount": 0}
    except Exception:
        level, spend = ({"name": "Member", "discount": 0}, 0)
    try:
    ...
    try:
        if isinstance(expiry, (int, float)):
            expiry_dt = datetime.fromtimestamp(float(expiry), tz=ZoneInfo("UTC"))
            expiry_text = format_bot_time(expiry_dt)
        elif isinstance(expiry, datetime):
            expiry_text = format_bot_time(expiry)
    except Exception:
        expiry_text = "No active premium"
    coin_name = _safe_text(settings.get("coin_name", "Coins"), "Coins")
    coin_emoji = _safe_text(settings.get("coin_emoji", "🪙"), "🪙")
    return (
        "👤 *My Profile*\n\n"
        f"👤 Name: *{_safe_text(message.from_user.first_name, 'User')}*\n"
        f"🆔 ID: `{uid}`\n\n"
        f"{coin_emoji} Balance: *{int(user.get('coins', 0) or 0)} {coin_name}*\n"
        f"💎 VIP Level: *{_safe_text(level.get('name', 'Member'))}*\n"
        f"💰 VIP Spending: *{int(spend or 0)}*\n"
        f"👥 Referrals: *{int(user.get('referral_count', 0) or 0)}*\n"
...
def final_achievements(message):
    ...
    try:
        ...
        u = {}
    try:
        level, _ = get_vip_level(uid)
        vip_name = level.get("name", "Member") if isinstance(level, dict) else "Member"
    except Exception:
        vip_name = "Member"
    try:
        streak = int((daily_streak_col.find_one({"user_id": uid}) or {}).get("streak", 0))
    ...
    text = (
        "🏅 *Achievements*\n\n"
        f"👥 Referrals: *{int(u.get('referral_count', 0) or 0)}*\n"
        f"🎯 Milestones: *{int(u.get('milestones_completed', 0) or 0)}*\n"
        f"💎 VIP: *{vip_name}*\n"
...
def final_rewards(message):
    ...
    try:
        ...
        row = {}
    try:
        cfg = advanced_settings() or {}
    except Exception:
        cfg = {}
    raw = cfg.get("streak_rewards", {}) or {}
    pairs = []
    if isinstance(raw, dict):
        for k, v in raw.items():
            try: pairs.append((int(k), int(v)))
            except Exception: pass
    pairs.sort(key=lambda x: x[0])
    text = "🎁 *Rewards*\n\n"
    text += f"🔥 Current streak: *{int(row.get('streak', 0) or 0)} days*\n\n"
    text += "🏆 Streak rewards:\n"
    text += "\n".join(f"• {d} days → {r} coins" for d, r in pairs) if pairs else "• No streak rewards configured yet."
    bot.send_message(message.chat.id, text, parse_mode="Markdown")
...
def advanced_lucky_spin(message):
    if not adv_flag("lucky_spin"):
        ...
        return
    uid = int(message.from_user.id)
    try: register_user(message.from_user)
    except Exception: pass
    now = adv_now()
    cfg = advanced_settings() or {}
    try: cooldown = max(0, int(cfg.get("spin_cooldown_hours", 24)))
    except Exception: cooldown = 24
    row = lucky_spin_col.find_one({"user_id": uid}) or {}
    extra = max(0, int(row.get("extra_spins", 0) or 0))
    last = row.get("last_spin")
    if isinstance(last, datetime):
        try:
            ...
            last = None
    if last is not None and cooldown > 0 and extra <= 0:
        elapsed = now - last
        if elapsed < timedelta(hours=cooldown):
            remaining = timedelta(hours=cooldown) - elapsed
            hours = int(remaining.total_seconds() // 3600)
            minutes = int((remaining.total_seconds() % 3600) // 60)
            bot.send_message(message.chat.id, f"⏳ Your next free spin is available in about *{hours}h {minutes}m*.\n\n🎡 Come back later!", parse_mode="Markdown")
            return
    rewards = cfg.get("spin_rewards", []) or []
    if not isinstance(rewards, list): rewards = []
    reward = weighted_spin(rewards)
    update = {"last_spin": now}
    if extra > 0: update["extra_spins"] = extra - 1
    lucky_spin_col.update_one({"user_id": uid}, {"$set": update, "$inc": {"total_spins": 1}}, upsert=True)
    kind = reward.get("type")
    try: value = int(reward.get("value", 0) or 0)
    except Exception: value = 0
    if kind == "coins" and value > 0:
        adv_add_coins(uid, value, "lucky spin")
        result = "🪙 Reward added to your balance."
    elif kind == "spin" and value > 0:
        lucky_spin_col.update_one({"user_id": uid}, {"$inc": {"extra_spins": value}}, upsert=True)
        result = "🔥 Extra spin added!"
    else:
        result = "😄 Better luck next time!"
    bot.send_message(message.chat.id, f"🎡 *Lucky Spin Result*\n\n🎉 You got: *{_safe_text(reward.get('label', 'Try Again'))}*\n\n{result}", parse_mode="Markdown")
...
def advanced_admin_hub(message):
    try:
        ...
        enabled = sum(1 for k in ADVANCED_DEFAULT_FLAGS if flags.get(k, True))
        deals = len(get_active_flash_deals())
        campaigns = referral_campaign_col.count_documents({"enabled": True})
        security = security_col.count_documents({})
        users = bot_users_col.count_documents({})
        text = ("🧠 *ADVANCED CONTROL CENTER*\n\n"
                f"🎛️ Advanced systems enabled: *{enabled}/{len(ADVANCED_DEFAULT_FLAGS)}*\n"
                f"⚡ Active flash deals: *{deals}*\n"
                f"🚀 Campaigns: *{campaigns}*\n"
                f"🛡️ Security events: *{security}*\n"
                f"👤 Registered users: *{users}*\n\n"
                "Everything below is controlled through Admin buttons.")
    except Exception as e:
        print(f"Advanced hub data error: {e}")
        text = "🧠 *ADVANCED CONTROL CENTER*\n\nChoose an advanced system below."
    bot.send_message(message.chat.id, text, reply_markup=advanced_admin_inline(), parse_mode="Markdown")


def advanced_rewards_menu(message):
    try: return advanced_rewards_control(message.chat.id)
    except Exception as e:
        print(f"Reward settings error: {e}")
        markup = InlineKeyboardMarkup(row_width=2)
        markup.row(InlineKeyboardButton("🪙 Daily Reward", callback_data="advset:daily"), InlineKeyboardButton("🔥 Streaks", callback_data="advset:streak"))
        markup.row(InlineKeyboardButton("🎡 Spin Cooldown", callback_data="advset:spin"), InlineKeyboardButton("🎡 Spin Rewards", callback_data="advset:spinrewards"))
        bot.send_message(message.chat.id, "🎁 *REWARD SETTINGS*\n\nAdvanced reward controls are ready.", reply_markup=markup, parse_mode="Markdown")


def advanced_vip_menu(message):
    try: return advanced_vip_control(message.chat.id)
    except Exception as e:
        print(f"VIP settings error: {e}")
        bot.send_message(message.chat.id, "💎 *VIP LEVELS*\n\nNo VIP data is available yet. You can replace the VIP levels below.", reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("✏️ Replace VIP Levels", callback_data="vipadmin:replace")), parse_mode="Markdown")


def advanced_security_menu(message):
    try: return advanced_security_control(message.chat.id)
    except Exception as e:
        print(f"Security settings error: {e}")
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("✏️ Set Referral Limit", callback_data="security:setlimit"))
        bot.send_message(message.chat.id, "🛡️ *ANTI-ABUSE CENTER*\n\nProtection controls are available below.", reply_markup=markup, parse_mode="Markdown")


def advanced_recovery_menu(message):
    try: return advanced_recovery_control(message.chat.id)
    except Exception as e:
        print(f"Recovery settings error: {e}")
        markup = InlineKeyboardMarkup(row_width=1)
        markup.add(InlineKeyboardButton("📣 Send Recovery Campaign", callback_data="recovery:send"))
        markup.add(InlineKeyboardButton("⚙️ Settings", callback_data="recovery:settings"))
        bot.send_message(message.chat.id, "♻️ *USER RECOVERY*\n\nRecovery controls are available below.", reply_markup=markup, parse_mode="Markdown")
