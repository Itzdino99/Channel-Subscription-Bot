...
def _paginate_keyboard(buttons, page, per_page=6, next_label=USER_NEXT, previous_label=USER_PREVIOUS):
...
def _user_menu_buttons(user_id=None):
    """Return the complete user menu in the order used by the paginator.

    This function intentionally does not call advanced_settings()/adv_flag();
    /start must never collapse to the four-button fallback because an optional
    advanced configuration is unavailable. Missing advanced flags default ON.
    """
    settings = get_settings()
    buttons = []
    ...
    for key, default in (
        ...
        label = settings.get(key, default)
        if key == "btn_refer" and not _safe_user_feature_enabled(settings, "referrals", True):
            continue
        if key == "btn_redeem" and not (
            _safe_user_feature_enabled(settings, "single_redeem", True)
            or _safe_user_feature_enabled(settings, "bulk_redeem", True)
        ):
            continue
        if key == "btn_coupon" and not (
            _safe_user_feature_enabled(settings, "coupons", True)
            or _safe_user_feature_enabled(settings, "personal_coupons", True)
        ):
            continue
        if key == "btn_referrals" and not _safe_user_feature_enabled(settings, "referrals", True):
            continue
        if key == "btn_milestones" and not _safe_user_feature_enabled(settings, "milestones", True):
            continue
        if key == "btn_leaderboard" and not _safe_user_feature_enabled(settings, "leaderboard", True):
            continue
        if key == "btn_feedback" and not _safe_user_feature_enabled(settings, "feedback", True):
            continue
        buttons.append(str(label))

    if _safe_user_feature_enabled(settings, "purchase_history", True):
        buttons.append(USER_HISTORY)

    # Advanced user systems. Missing flags intentionally mean enabled so an
    # older database cannot hide the new features.
    advanced_user_features = [
        ("daily_checkin", USER_DAILY),
        ("lucky_spin", USER_SPIN),
        ("vip_levels", USER_VIP),
        ("flash_deals", USER_DEALS),
        ("renewal", USER_RENEW),
        ("gift_premium", USER_GIFT),
    ]
    for flag_name, label in advanced_user_features:
        if _safe_user_feature_enabled(settings, flag_name, True):
            buttons.append(label)

    # Dashboard is informational and therefore always available.
    buttons.append(USER_DASHBOARD)

    if (
        _safe_user_feature_enabled(settings, "smart_notifications", True)
        or _safe_user_feature_enabled(settings, "notifications", True)
    ):
        buttons.append(USER_NOTIFICATIONS)

    if _safe_user_feature_enabled(settings, "referrals", True) and _safe_user_feature_enabled(settings, "referral_campaigns", True):
        # Referral campaigns are represented through Refer & Earn, so no extra
        # duplicate button is added.
        pass

    buttons.append(USER_STATUS)
    buttons.append(USER_HELP)

    # Wallet is intentionally omitted; the balance is shown in Profile/Dashboard.
    return buttons
...
def show_user_menu(chat_id):
    ...
    try:
        ...
        markup, _, _ = _paginate_keyboard(base, 0, per_page=6, next_label=USER_NEXT, previous_label=USER_PREVIOUS)
...
def user_menu_next(message):
    user_id = message.from_user.id
    buttons = _user_menu_buttons(user_id)
    total_pages = max(1, (len(buttons) + 5) // 6)
