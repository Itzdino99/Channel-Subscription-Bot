...
def admin_only(call_or_message):
    ...
    return uid == ADMIN_ID or admin_has_permission(uid, "users") or admin_has_permission(uid, "premium") or admin_has_permission(uid, "coins") or admin_has_permission(uid, "coupons") or admin_has_permission(uid, "broadcast") or admin_has_permission(uid, "statistics") or admin_has_permission(uid, "support") or admin_has_permission(uid, "announcements")
...
def support_ticket_user_router(message):
    if message.content_type == "text" and (message.text or "").strip() in {USER_PROFILE, USER_REFER, USER_EXTRA, USER_PREMIUM, USER_REFERRALS, USER_LEADERBOARD, USER_CONTACT, USER_DAILY, USER_COUPON, USER_MILESTONES, USER_ANNOUNCEMENTS, USER_FEEDBACK, USER_REDEEM, USER_GIFT, USER_RENEW, USER_BACK}:
        return
...
def final_user_pages(user_id):
    ...
    return [
        ...
        ([USER_EXTRA] if final_flag("extra_features") else []) + ([USER_PREMIUM] if final_flag("premium_menu") else []),
...
def final_user_keyboard(user_id, page=0):
    ...
    for row in pages[page]:
        pass
    # Page 0 is the requested main layout when all selected features are enabled.
    # Feature controls can hide individual sections without touching legacy code.
...
def premium_menu_keyboard():
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.row(KeyboardButton(USER_REDEEM))
    markup.row(KeyboardButton(USER_GIFT))
    markup.row(KeyboardButton(USER_RENEW))
...
def _profile_purchased_channels(user_id):
    ...
    for row in rows:
        ...
        name = channel.get("name") or row.get("channel_name") or "Premium Channel"
...
def _final_ui_router_match(message):
    ...
    labels = {
        ...
        USER_MY_REWARDS, USER_FEEDBACK, USER_CONTACT, USER_EXTRA, USER_PREMIUM, USER_BACK, USER_SPIN, USER_NEXT, USER_PREVIOUS,
...
def profile_expiry_callback(call):
    bot.answer_callback_query(call.id)
    final_expiry_info(call.message)
...
def _prioritize_advanced_handlers():
    ...
    priority = {
        ...
        "support_ticket_user_router": -20,
        "delegated_admin_command": -19,
        "delegated_admin_refresh": -18,
        "delegated_admin_feature": -17,
        "final_ui_router": -10,
    }
