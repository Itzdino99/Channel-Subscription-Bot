...
DEFAULT_SETTINGS = {
    ...
    # Fixed coin price for bulk Premium redemption. None means disabled.
...
def bulk_redeem_start(call):
    ...
    settings = get_settings()
    bulk_price = settings.get("bulk_redeem_price")

    if bulk_price is None:
        bot.answer_callback_query(
            call.id,
...
💰 *Fixed Price:* {bulk_price} {settings['coin_name']}
📢 You can select as many Premium channels as you want.
💡 The price stays the same regardless of the number of channels.
...
def set_bulk_redeem_price(call):
    ...
    current_price = get_settings().get("bulk_redeem_price")
    current_text = (
    ...
    msg = bot.send_message(
        ADMIN_ID,
        f"""📦 *Set Bulk Redeem Price*

Current fixed price: *{current_text}*

Send the coin price for Bulk Redeem.
The user will pay this amount only once, regardless of how many Premium channels they select.
...
Example: `500`""",
    ...
    bot.register_next_step_handler(
        msg,
        save_bulk_redeem_price
    )

    bot.answer_callback_query(call.id)


def save_bulk_redeem_price(message):
    ...
    try:
        ...
        bot.send_message(
            ADMIN_ID,
            "❌ Please send a valid coin amount (0 or more)."
        )
        return

    update_setting("bulk_redeem_price", price)

    bot.send_message(
        ADMIN_ID,
        f"""✅ *Bulk Redeem Price Updated!*

💰 Fixed Price: *{price} {get_settings()['coin_name']}*

Users can now select any number of Premium channels in Bulk Redeem and will pay this fixed price only once.""",
        parse_mode="Markdown"
    )
