# =========================================================
# TELEGRAM SUBSCRIPTION + REFERRAL + PREMIUM BOT
# FULL CONSOLIDATED VERSION
#
# FEATURES:
...
# ✅ Multiple Premium reward channels (add/remove)
# ✅ Premium redemption using coins
# ✅ Coupons
# ✅ Leaderboard
# ✅ Feedback
# ✅ Broadcast
...
# ✅ Editable texts, settings and user buttons
...
DEFAULT_SETTINGS = {
    ...
    # Legacy single reward channel compatibility
...
def is_admin_mode(user_id):
    ...
    return bool(
        user and user.get("admin_mode", False)
    )
...
def admin_mode_menu(chat_id):
    ...
    markup.row(
        ...
        KeyboardButton("🎁 Premium Channels")
    )

    markup.row(
        KeyboardButton("📢 Force Join"),
        KeyboardButton("🪙 Rewards & Coins")
    )
    ...
    markup.row(
        KeyboardButton("💬 Feedbacks"),
        KeyboardButton("⚙️ Settings")
    )

    markup.row(
        KeyboardButton("✏️ Content & Buttons"),
...
def show_force_join(chat_id):
    ...
    if not channels:
        bot.send_message(
            chat_id,
            "⚠️ Required verification channels have "
            "not been configured yet. Please contact the admin."
        )
        return
...
@bot.message_handler(
    func=lambda m:
    m.text == get_settings()["btn_profile"]
)
def my_profile(message):
    ...
    if user.get("referrer_id"):
        ...
        if referrer:
            referrer_text = user_display_name(
                referrer
            )
        else:
...
@bot.message_handler(
    func=lambda m:
    m.text == get_settings()["btn_referrals"]
)
...
@bot.message_handler(
    func=lambda m:
    m.text == get_settings()["btn_contact"]
)
...
@bot.message_handler(
    func=lambda m:
    m.text == get_settings()["btn_feedback"]
)
...
# =========================================================
# PREMIUM REWARD CHANNEL HELPERS
# =========================================================

def migrate_legacy_reward_channel():

    settings = get_settings()

    legacy_id = settings.get(
        "reward_channel_id"
    )
...
# =========================================================
# REDEEM PREMIUM - MULTIPLE CHANNELS
# =========================================================

@bot.message_handler(
    func=lambda m:
    m.text == get_settings()["btn_redeem"]
)
...
Choose the Premium channel you want to join:""",
        reply_markup=markup,
        parse_mode="Markdown"
    )
...
@bot.callback_query_handler(
    func=lambda c:
    c.data.startswith("rewardchannel_")
)
def select_reward_channel(call):
    ...
    try:
        ...
        if not channel:
            bot.answer_callback_query(
                call.id,
                "This Premium channel no longer exists.",
                show_alert=True
            )
            return
...
Choose your Premium duration:""",
        ...
        print(
            f"Reward channel selection error: {e}"
        )


@bot.callback_query_handler(
    func=lambda c:
    c.data.startswith("redeem_")
)
def redeem_premium(call):
    ...
    try:
        ...
        cost_map = {
            1: int(
                settings["reward_1_day_cost"]
            ),
            7: int(
                settings["reward_7_day_cost"]
            ),
            30: int(
                settings["reward_30_day_cost"]
            )
        }
...
@bot.message_handler(
    func=lambda m:
    m.text == get_settings()["btn_coupon"]
)
...
@bot.message_handler(
    func=lambda m:
    m.text == get_settings()["btn_leaderboard"]
)
...
@bot.message_handler(
    func=lambda m:
    m.from_user.id in pending_payments,
    content_types=["text"]
)
...
ADMIN_BUTTON_COMMANDS = {
    ...
    "🎁 Premium Channels": (
        ...
        "Use:\n"
        "`/setpremium` — Add Premium channel\n"
        "`/premiumlist` — View/remove channels"
    ),
    ...
    "📢 Broadcast": (
        "📢 Send `/broadcast` to start a broadcast."
    ),

    "📊 Statistics": (
        "📊 Send `/stats` to view bot statistics."
    ),

    "💬 Feedbacks": (
        "💬 Send `/feedbacks` to view feedback."
    ),

    "⚙️ Settings": (
        "⚙️ Send `/settings` to view current settings."
    ),
...
@bot.message_handler(
    func=lambda m:
    m.from_user.id == ADMIN_ID
    and m.text in ADMIN_BUTTON_COMMANDS
)
def admin_button_action(message):
...
📢 Premium Reward Channels: *{reward_channels_col.count_documents({})}*""",
        parse_mode="Markdown"
    )
...
def set_reward_cost(message):
    ...
    try:
        ...
        bot.reply_to(
            ...
            "and cost must be a number."
        )


# =========================================================
# EDITABLE TEXT / BUTTON SYSTEM
# =========================================================
...
def force_add_start(message):
    ...
    msg = bot.send_message(
        ADMIN_ID,
        "📢 Forward a message from the channel/group users "
        "must join.\n\n"
...
@bot.callback_query_handler(
    func=lambda c:
    c.data.startswith("force_remove_")
)
...
# =========================================================
# MULTIPLE PREMIUM CHANNEL ADMIN
# =========================================================
...
def save_premium_channel(message):
    ...
    # Keep first channel compatible with old system
    settings = get_settings()
...
You can add more Premium channels anytime using `/setpremium`.""",
        parse_mode="Markdown"
    )


@bot.message_handler(
    commands=["premiumlist"],
    func=lambda m:
    m.from_user.id == ADMIN_ID
)
def premium_list(message):
    ...
    if not channels:
        ...
        bot.send_message(
            ADMIN_ID,
            "🎁 No Premium reward channels have been added."
        )
        return
...
@bot.callback_query_handler(
    func=lambda c:
    c.data.startswith("premium_remove_")
)
def remove_premium_channel(call):
    ...
    try:
        ...
        if (
            ...
            remaining = list(
                reward_channels_col.find().limit(1)
            )
        ...
        bot.edit_message_text(
            "✅ Premium reward channel removed.",
            call.message.chat.id,
            call.message.message_id
        )
...
⏰ Expires in: {hours} hours""",
        ...
        bot.reply_to(
            message,
            "❌ Values must be valid positive numbers."
        )
...
🎁 Premium Channels: *{reward_channels_col.count_documents({})}*
...
🔄 *Modes*
`/admin` — Open Admin Mode
Use *User Mode* button to return to normal user mode.""",
        parse_mode="Markdown"
    )
...
def kick_expired_users():
    ...
    for user in expired_users:
        ...
        try:
            ...
            # Removes member but allows future rejoining
            bot.ban_chat_member(
                channel_id,
                user_id
            )
...
🪙 Earn more coins through referrals and redeem Premium again!""",
            ...
            # Record stays in database so scheduler retries
            print(
...
def unknown_message(message):
    ...
    # Don't show fallback while admin is using admin mode
    if (
        message.from_user.id == ADMIN_ID
        and is_admin_mode(ADMIN_ID)
    ):

        bot.send_message(
            message.chat.id,
            "👑 Please use the Admin Mode buttons or `/helpadmin`."
        )
        return
...
if __name__ == "__main__":
    ...
    # Automatically migrate old single Premium channel
    migrate_legacy_reward_channel()
