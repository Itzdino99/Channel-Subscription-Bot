...
def show_admin_panel(chat_id):
    ...
    # Reset to the first admin page whenever the admin panel is explicitly opened.
    _admin_menu_pages[chat_id] = 0 if "_admin_menu_pages" in globals() else 0
    settings = get_settings()
...
Select a subscription plan below.""",
    ...
    try:
        ...
        try:
            ...
            fallback.row(KeyboardButton(get_settings().get("btn_profile", "👤 Profile")), KeyboardButton(get_settings().get("btn_redeem", "💎 Redeem Premium")))
            fallback.row(KeyboardButton(get_settings().get("btn_refer", "🎁 Refer & Earn")), KeyboardButton(get_settings().get("btn_coupon", "🎟️ Claim Coupon")))
...
def _is_advanced_keyboard_text(message):
    ...
    admin_labels = {
        ...
        globals().get("ADMIN_REWARDS"), globals().get("ADMIN_VIP"),
        globals().get("ADMIN_DEALS"), globals().get("ADMIN_GROWTH"),
        globals().get("ADMIN_SECURITY"), globals().get("ADMIN_NOTIFY"),
        globals().get("ADMIN_RECOVERY"), globals().get("ADMIN_CAMPAIGNS"),
...
def _user_menu_buttons(user_id=None):
    """Complete user menu, ordered so the most important earning actions stay on page 1."""
    settings = get_settings()
    buttons = []

    def enabled(name, default=True):
        return _safe_user_feature_enabled(settings, name, default)

    def add(key, default, flag=None):
        if flag and not enabled(flag, True):
            return
        buttons.append(str(settings.get(key, default)))

    # MAIN PAGE PRIORITY: Daily Check-in and Gift Premium are intentionally
    # near the top, and Earn Extra is always adjacent to the earning actions.
    add("btn_profile", "🌐 My Profile")
    if enabled("single_redeem", True) or enabled("bulk_redeem", True):
        add("btn_redeem", "🎁 Redeem Premium")
    add("btn_refer", "🔗 Refer & Earn", "referrals")
    if enabled("daily_checkin", True) and enabled("daily_bonus", True):
        buttons.append(USER_DAILY)
    if enabled("gift_premium", True):
        buttons.append(USER_GIFT)
    if any(enabled(x, True) for x in ("daily_checkin", "lucky_spin", "flash_deals", "referrals", "milestones")):
        buttons.append(USER_EARN_EXTRA)
    add("btn_coupon", "🎟️ Claim Coupon", "coupons")
    add("btn_referrals", "👥 My Referrals", "referrals")

    # Remaining original features.
    add("btn_milestones", "🎯 Milestones", "milestones")
    add("btn_leaderboard", "🏆 Leaderboard", "leaderboard")
    add("btn_how", "📖 How It Works")
    add("btn_feedback", "💬 Feedback", "feedback")
    add("btn_contact", "📞 Contact Admin")

    if enabled("purchase_history", True):
        buttons.append(USER_HISTORY)
    if enabled("lucky_spin", True):
        buttons.append(USER_SPIN)
    if enabled("vip_levels", True):
        buttons.append(USER_VIP)
    if enabled("flash_deals", True):
        buttons.append(USER_DEALS)
    if enabled("renewal", True):
        buttons.append(USER_RENEW)

    buttons.append(USER_DASHBOARD)
    if enabled("smart_notifications", True) or enabled("notifications", True):
        buttons.append(USER_NOTIFICATIONS)
    buttons.append(USER_STATUS)
    buttons.append(USER_HELP)

    # Wallet intentionally omitted; Profile/Dashboard show the balance.
    return buttons


def ultimate_user_menu_markup(user_id=None, page=None):
    if page is None:
        page = _user_menu_pages.get(user_id, 0)
    buttons = _user_menu_buttons(user_id)
    markup, page, total_pages = _paginate_keyboard(
        buttons,
        page,
        per_page=8,
        next_label=USER_NEXT,
        previous_label=USER_PREVIOUS,
    )
    if user_id is not None:
        _user_menu_pages[user_id] = page
    return markup
...
def show_user_menu(chat_id):
    """Always show the full paginated user menu; never fall back to four buttons."""
    _user_menu_pages[chat_id] = 0
    settings = get_settings()
    try:
        markup = ultimate_user_menu_markup(chat_id, 0)
    except Exception as e:
        # Last-resort menu builder. It still exposes the full user feature set
        # and pagination even if a Mongo/settings value is temporarily malformed.
        print(f"USER MENU BUILD ERROR for {chat_id}: {e}")
        base = [
            settings.get("btn_profile", "🌐 My Profile"),
            settings.get("btn_redeem", "🎁 Redeem Premium"),
            settings.get("btn_refer", "🔗 Refer & Earn"),
            settings.get("btn_coupon", "🎟️ Claim Coupon"),
            settings.get("btn_referrals", "👥 My Referrals"),
            settings.get("btn_milestones", "🎯 Milestones"),
            settings.get("btn_leaderboard", "🏆 Leaderboard"),
            settings.get("btn_how", "📖 How It Works"),
            settings.get("btn_feedback", "💬 Feedback"),
            settings.get("btn_contact", "📞 Contact Admin"),
            USER_HISTORY, USER_DAILY, USER_SPIN, USER_VIP, USER_DASHBOARD,
            USER_DEALS, USER_RENEW, USER_GIFT, USER_NOTIFICATIONS, USER_STATUS, USER_HELP,
        ]
        markup, _, _ = _paginate_keyboard(base, 0, per_page=8, next_label=USER_NEXT, previous_label=USER_PREVIOUS)
...
def advanced_feature_control(chat_id):
    ...
    bot.send_message(chat_id, "🎛️ *FEATURE CONTROLS*\n\nTap any button to turn any user-visible or advanced feature ON/OFF. Changes apply immediately to the user panel.", reply_markup=final_feature_keyboard(), parse_mode="Markdown")
...
def advanced_renew_menu(message):
    ...
    for row in active[:10]:
        text += f"📢 Channel: `{row.get('channel_id')}`\n⏰ Expires: *{format_bot_time(datetime.fromtimestamp(float(row.get('expiry')), tz=ZoneInfo('UTC')))}*\n\n"
    bot.send_message(message.chat.id, text, parse_mode="Markdown")
...
# Exact final user layout.  The first page intentionally contains the user's
# most important earning/premium actions; subsequent pages stay compact.
def final_user_pages(user_id):
    ensure_final_feature_flags()
    pages = []

    page1 = []
    if final_flag("profile"): page1.append(USER_PROFILE)
    if final_flag("premium_redeem") and (final_flag("single_redeem") or final_flag("bulk_redeem")): page1.append(USER_REDEEM)
    if final_flag("referrals"): page1.append(USER_REFER)
    if final_flag("gift_premium"): page1.append(USER_GIFT)
    if final_flag("daily_checkin") and final_flag("daily_bonus"): page1.append(USER_DAILY)
    if final_flag("earn_extra"): page1.append(USER_EARN_EXTRA)
    if final_flag("coupons") or final_flag("personal_coupons"): page1.append(USER_COUPON)
    if final_flag("my_referrals"): page1.append(USER_REFERRALS)
    pages.append(page1)

    page2 = []
    if final_flag("vip_levels"): page2.append(USER_VIP)
    if final_flag("rewards"): page2.append(USER_REWARDS)
    if final_flag("milestones"): page2.append(USER_MILESTONES)
    if final_flag("leaderboard"): page2.append(USER_LEADERBOARD)
    if final_flag("flash_deals"): page2.append(USER_DEALS)
    if final_flag("smart_notifications") or final_flag("notifications"): page2.append(USER_NOTIFICATIONS)
    pages.append(page2)

    page3 = []
    if final_flag("purchase_history"): page3.append(USER_HISTORY)
    if final_flag("announcements"): page3.append(USER_ANNOUNCEMENTS)
    if final_flag("renewal"): page3.append(USER_RENEW)
    if final_flag("expiry_info"): page3.append(USER_EXPIRY_INFO)
    if final_flag("how_it_works"): page3.append(USER_HOW)
    if final_flag("help_rules"): page3.append(USER_HELP)
    pages.append(page3)
...
def final_user_keyboard(user_id, page=0):
    pages = final_user_pages(user_id)
    if not pages:
        pages = [[USER_PROFILE]]
    page = max(0, min(int(page), len(pages) - 1))
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    for i in range(0, len(pages[page]), 2):
        markup.row(*[KeyboardButton(x) for x in pages[page][i:i+2]])
    nav = []
    if page > 0: nav.append(KeyboardButton(USER_PREVIOUS))
    if page < len(pages)-1: nav.append(KeyboardButton(USER_NEXT))
    if nav: markup.row(*nav)
    return markup, page, len(pages)
...
# Earning submenu requested by the user.  It has its own Previous/Back path.
def final_earn_extra_keyboard(user_id):
    items = []
    if final_flag("daily_checkin") and final_flag("daily_bonus"): items.append(USER_DAILY)
    if final_flag("lucky_spin"): items.append(USER_SPIN)
    if final_flag("referrals"): items.append(USER_REFER)
    if final_flag("milestones"): items.append(USER_MILESTONES)
    if final_flag("coupons") or final_flag("personal_coupons"): items.append(USER_COUPON)
    if final_flag("flash_deals"): items.append(USER_DEALS)
    if final_flag("achievements"): items.append(USER_ACHIEVEMENTS)
    if final_flag("referral_campaigns"): items.append(USER_REFERRAL_CAMPAIGNS)
    if final_flag("my_rewards"): items.append(USER_MY_REWARDS)
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    for i in range(0, len(items), 2):
        markup.row(*[KeyboardButton(x) for x in items[i:i+2]])
    markup.row(KeyboardButton(USER_PREVIOUS))
    return markup
...
def final_profile_text(message):
    ...
    cfg = advanced_settings()
    vip_result = get_vip_level(uid) if 'get_vip_level' in globals() else ({"name":"Member","spend":0,"discount":0}, 0)
    level, vip_total = vip_result if isinstance(vip_result, tuple) else (vip_result, 0)
    lifetime = int(vip_total or 0)
    try:
        streak = int((daily_streak_col.find_one({"user_id": uid}) or {}).get("streak", 0))
    except Exception: streak = 0
    expiry = user.get("expiry")
    expiry_text = "No active premium"
    if expiry:
        try:
            if isinstance(expiry, (int, float)):
                expiry_dt = datetime.fromtimestamp(float(expiry), tz=bot_time_now().tzinfo)
            elif isinstance(expiry, datetime):
                expiry_dt = expiry
            else: expiry_dt = None
            if expiry_dt: expiry_text = format_bot_time(expiry_dt)
        except Exception: pass
    s = get_settings()
    return (f"👤 *My Profile*\n\n"
            f"👤 Name: *{message.from_user.first_name or 'User'}*\n"
            f"🆔 ID: `{uid}`\n\n"
            f"{s.get('coin_emoji','🪙')} Balance: *{user.get('coins',0)} {s.get('coin_name','Coins')}*\n"
            f"💎 VIP Level: *{level.get('name','Member')}*\n"
            f"💰 VIP Spending: *{lifetime}*\n"
            f"👥 Referrals: *{user.get('referral_count',0)}*\n"
            f"🔥 Streak: *{streak} days*\n"
            f"🎁 Premium: *{expiry_text}*\n"
            f"🔔 Expiry reminders: *{'ON' if final_flag('premium_expiry_notice') else 'OFF'}*\n\n"
            f"📊 Your dashboard, status, wallet balance and VIP spending statistics are included here.")
...
def _final_ui_router_match(message):
    if not getattr(message, "text", None) or getattr(message, "content_type", None) != "text":
        return False
    labels = {
        USER_PROFILE, USER_REDEEM, USER_REFER, USER_GIFT, USER_DAILY, USER_EARN_EXTRA,
        USER_COUPON, USER_REFERRALS, USER_VIP, USER_REWARDS, USER_MILESTONES, USER_LEADERBOARD,
        USER_DEALS, USER_NOTIFICATIONS, USER_HISTORY, USER_ANNOUNCEMENTS, USER_RENEW,
        USER_EXPIRY_INFO, USER_HOW, USER_HELP, USER_ACHIEVEMENTS, USER_REFERRAL_CAMPAIGNS,
        USER_MY_REWARDS, USER_FEEDBACK, USER_CONTACT, USER_SPIN, USER_NEXT, USER_PREVIOUS,
        ADMIN_NEXT, ADMIN_PREVIOUS, ADMIN_ADVANCED, ADMIN_FEATURES, ADMIN_REWARDS, ADMIN_VIP,
        ADMIN_DEALS, ADMIN_GROWTH, ADMIN_SECURITY, ADMIN_NOTIFY, ADMIN_RECOVERY,
        ADMIN_CAMPAIGNS, ADMIN_BROADCAST_TARGET, ADMIN_GIFTS,
    }
...
def final_ui_router(message):
    ...
    uid = message.from_user.id
    if uid == ADMIN_ID:
        if text == ADMIN_NEXT: return admin_menu_next(message)
        if text == ADMIN_PREVIOUS: return admin_menu_previous(message)
        admin_routes = {
            ADMIN_ADVANCED: advanced_admin_hub,
            ADMIN_FEATURES: advanced_feature_menu,
            ADMIN_REWARDS: advanced_rewards_menu,
        ...
        fn = admin_routes.get(text)
        if fn:
            try: return fn(message)
            except Exception as e:
                print(f"Admin feature '{text}' error: {e}")
                bot.send_message(message.chat.id, "⚠️ This section encountered an error. Please try again.")
                return
        return

    if text == USER_NEXT:
        pages=final_user_pages(uid); cur=_user_menu_pages.get(uid,0); _user_menu_pages[uid]=min(cur+1,max(0,len(pages)-1)); return final_show_user_menu(uid,_user_menu_pages[uid],f"📱 *Menu* — Page {_user_menu_pages[uid]+1}/{len(pages)}")
    if text == USER_PREVIOUS:
        pages=final_user_pages(uid); cur=_user_menu_pages.get(uid,0); _user_menu_pages[uid]=max(0,cur-1); return final_show_user_menu(uid,_user_menu_pages[uid],f"📱 *Menu* — Page {_user_menu_pages[uid]+1}/{len(pages)}")
    routes = {
        USER_PROFILE: lambda m: bot.send_message(m.chat.id, final_profile_text(m), parse_mode="Markdown"),
        USER_REDEEM: redeem_premium_menu,
        USER_REFER: refer_and_earn,
        USER_COUPON: claim_coupon_prompt,
        USER_REFERRALS: my_referrals,
        USER_EARN_EXTRA: lambda m: bot.send_message(m.chat.id, "💰 *Earn Extra*\n\nChoose an earning method below:", reply_markup=final_earn_extra_keyboard(uid), parse_mode="Markdown"),
        USER_REWARDS: final_rewards,
        USER_MY_REWARDS: final_my_rewards,
        USER_ANNOUNCEMENTS: final_announcements,
        USER_EXPIRY_INFO: final_expiry_info,
        USER_ACHIEVEMENTS: final_achievements,
        USER_REFERRAL_CAMPAIGNS: final_referral_campaigns,
        USER_DAILY: advanced_daily_checkin,
        USER_SPIN: advanced_lucky_spin,
        USER_VIP: advanced_vip,
        USER_DASHBOARD: advanced_dashboard,
        ...
        USER_GIFT: gift_premium_start,
        USER_RENEW: advanced_renew_menu,
        USER_MILESTONES: show_milestones,
        USER_LEADERBOARD: leaderboard,
        USER_HOW: lambda m: bot.send_message(m.chat.id, get_settings().get("how_it_works_text", "Follow the instructions to earn rewards and redeem premium.")),
        USER_FEEDBACK: feedback_start,
        USER_CONTACT: contact_admin,
        USER_HISTORY: advanced_user_history_handler,
        USER_HELP: advanced_user_help_handler,
    }
    fn = routes.get(text)
    if fn:
        try: return fn(message)
        except Exception as e:
            print(f"User feature '{text}' error: {e}")
            bot.send_message(message.chat.id, "⚠️ This feature encountered an error. Please try again.")


# Override the runtime menu functions used by /start and by all existing core flows.
def user_menu_markup(user_id=None):
    uid = user_id
    pages = final_user_pages(uid)
    if not pages: pages = [[USER_PROFILE]]
    page = _user_menu_pages.get(uid, 0) if uid is not None else 0
    markup, page, _ = final_user_keyboard(uid, page)
    if uid is not None: _user_menu_pages[uid] = page
    return markup
...
def show_user_menu(chat_id):
    ensure_final_feature_flags()
    _user_menu_pages[chat_id] = 0
    markup, _, _ = final_user_keyboard(chat_id, 0)
    bot.send_message(chat_id, get_settings().get("welcome_text", "Choose an option:"), reply_markup=markup, parse_mode="Markdown")
...
def _prioritize_advanced_handlers():
    ...
    priority = {
        # Requested UI-only overrides must win over every legacy/advanced
        # keyboard handler without deleting any of those handlers.
        "requested_profile_handler": -30,
        "requested_extra_features_handler": -30,
        "requested_premium_handler": -30,
        "requested_user_back_handler": -30,
        "requested_admin_premium_channels": -30,
        "requested_admin_coins_referrals": -30,
        "requested_admin_rewards": -30,
        "requested_admin_broadcast": -30,
        "requested_admin_offers": -30,
        "requested_admin_settings": -30,
        "requested_admin_statistics": -30,
        "requested_admin_logs": -30,
        "final_ui_router": -10,
        "advanced_keyboard_priority_router": 0,
        # Keep /start and core navigation ahead of the generic fallback too.
        "start_handler": 1,
...
# =========================================================
# REQUESTED UI-ONLY OVERRIDE LAYER
# =========================================================
# IMPORTANT: This layer changes only which existing features are exposed by
# the keyboards/routing. Existing handlers, database logic, Force Join,
# payment logic, premium expiry cleanup, and other old code remain intact.
...
# Requested Admin Panel labels. Existing old admin constants/handlers are kept.
ADMIN_PREMIUM_CHANNELS = "📺 Premium & Channels"
ADMIN_COINS_REFERRALS = "🪙 Coins & Referrals"
ADMIN_REWARDS = "🎁 Rewards"
ADMIN_SUPPORT_TICKETS = "🎟️ Support Tickets"
ADMIN_BROADCAST_ANNOUNCEMENTS = "📢 Broadcast & Announcements"
ADMIN_OFFERS = "🏷️ Offers & Discounts"
ADMIN_BOT_SETTINGS = "⚙️ Bot Settings"
ADMIN_STATISTICS = "📊 Statistics"
ADMIN_ACTIVITY_LOGS = "📜 Activity Logs"


def requested_user_main_keyboard(user_id=None):
    """Only expose the user features requested in the agreed main layout."""
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
...
def requested_extra_keyboard():
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.row(KeyboardButton(USER_DAILY), KeyboardButton(USER_COUPON))
    markup.row(KeyboardButton(USER_MILESTONES), KeyboardButton(USER_ANNOUNCEMENTS))
    markup.row(KeyboardButton(USER_FEEDBACK), KeyboardButton(USER_BACK))
    return markup


def requested_premium_keyboard():
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.row(KeyboardButton(USER_REDEEM), KeyboardButton(USER_GIFT))
    markup.row(KeyboardButton(USER_RENEW), KeyboardButton(USER_BACK))
    return markup


def requested_admin_keyboard():
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.row(KeyboardButton(ADMIN_USERS), KeyboardButton(ADMIN_PREMIUM_CHANNELS))
    markup.row(KeyboardButton(ADMIN_COINS_REFERRALS), KeyboardButton(ADMIN_REWARDS))
    markup.row(KeyboardButton(ADMIN_SUPPORT_TICKETS), KeyboardButton(ADMIN_BROADCAST_ANNOUNCEMENTS))
    markup.row(KeyboardButton(ADMIN_COUPONS), KeyboardButton(ADMIN_OFFERS))
    markup.row(KeyboardButton(ADMIN_BOT_SETTINGS), KeyboardButton(ADMIN_STATISTICS))
    markup.row(KeyboardButton(ADMIN_ACTIVITY_LOGS))
    markup.row(KeyboardButton(ADMIN_MODE))
    return markup


def requested_profile_text(message):
    """Profile text requested by the user, including independently expiring purchased channels."""
    uid = message.from_user.id
    user = get_user(uid) or {}
    settings = get_settings()
    now_ts = bot_time_now().timestamp()
    ...
    try:
        joined_text = format_bot_time(joined) if joined else "Unknown"
    except Exception:
        joined_text = str(joined) if joined else "Unknown"

    referrer_id = user.get("referrer_id")
    if referrer_id:
        referrer = get_user(referrer_id) or {}
        referrer_text = f"{referrer.get('first_name') or referrer.get('username') or referrer_id} (`{referrer_id}`)"
    else:
        referrer_text = "None"

    rows = list(users_col.find({"user_id": uid}).sort("expiry", DESCENDING))
    active_rows = [r for r in rows if float(r.get("expiry", 0) or 0) > now_ts]

    if active_rows:
        channels = []
        for r in active_rows:
            channel_id = r.get("channel_id")
            channel = premium_channels_col.find_one({"channel_id": channel_id}) if channel_id is not None else None
            channel_name = (channel or {}).get("name") or (channel or {}).get("title") or str(channel_id or "Premium Channel")
            try:
                expiry_text = format_bot_time(datetime.fromtimestamp(float(r.get("expiry")), tz=bot_time_now().tzinfo))
            except Exception:
                expiry_text = str(r.get("expiry"))
            channels.append(f"• *{channel_name}* — ⏰ {expiry_text}")
        purchased_text = "\n".join(channels)
        premium_status = "Active"
    else:
        purchased_text = "None"
        premium_status = "No active premium"

    return (
        "👤 *My Profile*\n\n"
        f"👤 Name: *{message.from_user.first_name or user.get('first_name') or 'User'}*\n"
        f"🆔 Telegram ID: `{uid}`\n"
        f"📅 Joined Date: *{joined_text}*\n\n"
        f"{settings.get('coin_emoji', '🪙')} Coins: *{user.get('coins', 0)} {settings.get('coin_name', 'Coins')}*\n"
...
def requested_show_user_menu(chat_id):
    bot.send_message(
        chat_id,
        get_settings().get("welcome_text", "Choose an option:"),
        reply_markup=requested_user_main_keyboard(chat_id),
        parse_mode="Markdown",
    )


def requested_admin_show(chat_id):
    if chat_id != ADMIN_ID:
        return
    bot.send_message(
        chat_id,
        "👑 *ADMIN PANEL*\n\nChoose a section:",
        reply_markup=requested_admin_keyboard(),
        parse_mode="Markdown",
    )


# Override only the exposed keyboards. All original feature functions remain.
def user_menu_markup(user_id=None):
    return requested_user_main_keyboard(user_id)


def show_user_menu(chat_id):
    requested_show_user_menu(chat_id)


def admin_menu_markup():
    return requested_admin_keyboard()


def show_admin_panel(chat_id):
    requested_admin_show(chat_id)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id != ADMIN_ID and m.text == USER_EXTRA)
def requested_extra_features_handler(message):
    bot.send_message(message.chat.id, "🪄 *Extra Features*\n\nChoose an option:", reply_markup=requested_extra_keyboard(), parse_mode="Markdown")


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id != ADMIN_ID and m.text == USER_PREMIUM)
def requested_premium_handler(message):
    bot.send_message(message.chat.id, "🎁 *Premium*\n\nChoose an option:", reply_markup=requested_premium_keyboard(), parse_mode="Markdown")


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id != ADMIN_ID and m.text == USER_BACK)
def requested_user_back_handler(message):
    requested_show_user_menu(message.chat.id)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id != ADMIN_ID and m.text == USER_PROFILE)
def requested_profile_handler(message):
    bot.send_message(message.chat.id, requested_profile_text(message), parse_mode="Markdown")


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_PREMIUM_CHANNELS)
def requested_admin_premium_channels(message):
    # Existing Premium/Channels systems are preserved; this button is the new entry point.
    try:
        return admin_premium_menu(message)
    except Exception:
        return admin_channels_menu(message)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_COINS_REFERRALS)
def requested_admin_coins_referrals(message):
    bot.send_message(message.chat.id, "🪙 *Coins & Referrals*\n\nChoose an existing management action:", parse_mode="Markdown")
    try:
        return coin_add_prompt(message)
    except Exception:
        return


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_REWARDS)
def requested_admin_rewards(message):
    return advanced_rewards_menu(message)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_BROADCAST_ANNOUNCEMENTS)
def requested_admin_broadcast(message):
    return broadcast_message(message)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_OFFERS)
def requested_admin_offers(message):
    return advanced_bulk_rules_handler(message)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_BOT_SETTINGS)
def requested_admin_settings(message):
    return admin_settings_menu(message)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_STATISTICS)
def requested_admin_statistics(message):
    return admin_statistics_handler(message)


@bot.message_handler(func=lambda m: m.content_type == "text" and m.from_user.id == ADMIN_ID and m.text == ADMIN_ACTIVITY_LOGS)
def requested_admin_logs(message):
    return advanced_audit_handler(message)
