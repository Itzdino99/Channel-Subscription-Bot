...
def _user_menu_buttons(user_id=None):
    ...
    # Original user functions remain available, but are now paginated.
    buttons.extend([
        settings["btn_profile"],
    ])

    if feature_enabled("referrals", True):
        buttons.append(settings["btn_refer"])

    if feature_enabled("single_redeem", True) or feature_enabled("bulk_redeem", True):
        buttons.append(settings["btn_redeem"])

    if feature_enabled("coupons", True):
        buttons.append(settings["btn_coupon"])

    if feature_enabled("referrals", True):
        buttons.append(settings["btn_referrals"])

    if feature_enabled("milestones", True):
        buttons.append(settings["btn_milestones"])

    if feature_enabled("leaderboard", True):
        buttons.append(settings["btn_leaderboard"])

    buttons.append(settings["btn_how"])

    if feature_enabled("feedback", True):
        buttons.append(settings["btn_feedback"])

    buttons.append(settings["btn_contact"])

    if feature_enabled("purchase_history", True):
    ...
    # Personal dashboard is intentionally always available because it only reads data.
    ...
    if adv_flag("smart_notifications") or feature_enabled("notifications", True):
        buttons.append(USER_NOTIFICATIONS)

    # Legacy status/help are retained as normal user tools.
    buttons.append(USER_STATUS)
    buttons.append(USER_HELP)

    # Wallet is deliberately NOT included: balance is already visible in Profile/Dashboard.
    return buttons
...
def show_user_menu(chat_id):
    ...
    bot.send_message(
        chat_id,
        settings["welcome_text"],
        reply_markup=ultimate_user_menu_markup(chat_id, 0),
        parse_mode="Markdown"
    )
