...
💰 The reward has been automatically added to your balance!""",
        ...
        user_data = get_user(user_id)
        send_referral_notification_log(
            f"""🎯 *Milestone Completed*
...
def support_ticket_user_router(message):
    ...
    panel_labels = {USER_PROFILE, USER_REFER, USER_EXTRA, USER_PREMIUM, USER_REFERRALS, USER_LEADERBOARD, USER_CONTACT,
                    ...
                    USER_REDEEM, USER_GIFT, USER_RENEW, USER_HISTORY, USER_BACK}
...
def support_ticket_user_router(message):
    if message.content_type == "text" and (message.text or "").strip() in {USER_PROFILE, USER_REFER, USER_EXTRA, USER_PREMIUM, USER_REFERRALS, USER_LEADERBOARD, USER_CONTACT, USER_DAILY, USER_COUPON, USER_MILESTONES, USER_ANNOUNCEMENTS, USER_FEEDBACK, USER_REDEEM, USER_GIFT, USER_RENEW, USER_HISTORY, USER_BACK}:
        return
...
def final_broadcast_menu(chat_id=ADMIN_ID):
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.row(
        InlineKeyboardButton("📢 Broadcast to All", callback_data="finalbroadcast:all"),
...
def final_broadcast_all_callback(call):
    ...
    if not feature_enabled("broadcast", True):
        bot.send_message(ADMIN_ID, "⚠️ Broadcast is disabled in Feature Control.")
        return
    msg = bot.send_message(ADMIN_ID, "📢 Send the message to broadcast to all eligible users. You can send text, photo, video, document, etc.")
    bot.register_next_step_handler(msg, final_broadcast_all_receive)


def final_broadcast_all_receive(message):
    if message.from_user.id != ADMIN_ID:
        return
    sent = failed = 0
    for row in bot_users_col.find({"banned": {"$ne": True}}, {"user_id": 1}):
    ...
    announcement_col.insert_one({
        ...
        "failed": failed,
        "created_at": bot_time_now(),
        "source": "final_admin_broadcast",
    })
    write_audit(ADMIN_ID, "final_broadcast", {"sent": sent, "failed": failed, "content_type": message.content_type})
...
def final_broadcast_history_callback(call):
    bot.answer_callback_query(call.id)
    rows = list(announcement_col.find().sort("created_at", DESCENDING).limit(20))
    if not rows:
    ...
    for row in rows:
        lines.append(
            f"• {format_bot_time(row.get('created_at'))} — {row.get('content_type', 'text')} — "
